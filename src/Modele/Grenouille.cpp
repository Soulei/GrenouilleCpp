...
namespace grenouilloland{
  ...
  void
  Grenouille::reduirePtvie(const Element & e) const{
  ...
  bool
  Grenouille::cheminExiste(const Jeu & jeu) const{
    ...
    if(x > 0 && y > 0 || x < g->getDimension() && y < g->getDimension()){
      if(jeu.lireGrille()[x*dim+y+1].couleurElt() == None){
        if(jeu.lireGrille()[(x+1)*dim+y].couleurElt() == None){
          if(jeu.lireGrille()[x*dim+y-1].couleurElt() == None){
            if(jeu.lireGrille()[(x-1)*dim+y].couleurElt() == None){
